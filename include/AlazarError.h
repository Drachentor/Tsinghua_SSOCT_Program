...
enum RETURN_CODE {
    ...
    /// The number of different DMA buffers posted exceeds the limit set with
    ...
    /// The operation could not complete because the system is in an invalid
    ...
    /// The operation could not complete because the system is in a busy
    /// state. You may safely retry the call that returned this error.
    ApiBusy = 617,

    /// Under Linux, ATSApi looks for an `ALAZARRCPATH` or a `HOME` environment
    ...
    /// The library does not support FPGA versions under 4.07 for ATS9364.
    ...
    /// The library does not support FPGA versions under 6.0 for ATS4001.
    /// Frimware must be updated.
